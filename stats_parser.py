...
def parse_victims_table(victims_table):
    victims = []
    death_info = None
...
def parse_stats_table(soup):
    ...
    for row in rows:
        ...
        if victim_div:
            ...
            if victims_table:
                victims, death = parse_victims_table(victims_table)
...
def fetch_and_update_stats(mission):
    ...
    if response.status_code != 200:
        print(f"Ошибка загрузки статистики: {response.status_code} для миссии {mission['mission_name']}")
        return mission

    soup = BeautifulSoup(response.text, 'html.parser')
    players_stats = parse_stats_table(soup)
